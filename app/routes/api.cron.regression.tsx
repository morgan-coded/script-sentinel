...
/**
 * Continuous Regression cron handler.
 *
 ...
 *
 * The first version RENDERS the email and logs it — actual delivery is left to the
 ...
 * merchant-driven action — see the README's honest-limitations note.
 */
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  for (const shop of shops) {
    try {
      // Review-response: only paying merchants get cron-driven runs.
      ...
      if (outcome.alerts.length > 0) {
        ...
        emailRendered = true;
        // Log the rendered email for the deployer's mail service
