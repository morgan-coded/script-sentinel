 ...
 * Diff engine.
 *
 * Joins `FixtureBaseline` rows (Script-era observations) to
 ...
 * tags*: they describe **who** is affected (B2B-tagged
...
export interface DiffBaselineInput {
  ...
  presentmentCurrency: string;
  /**
   * Optional segment context. When supplied, `runDiff` appends
   * the corresponding segment tag (`"b2b"` and/or `"market"`) to any
   * emitted DriftResult.categories. Absent → behaves like the diff-only path.
   */
...
export interface DiffEngineResult {
   ...
   * before cutover." Per the README's honest-limitation paragraph.
   */
 ...
 *   and != "US". The product premise is US/USD-default; any
