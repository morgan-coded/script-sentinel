/**
 * Server-side helper that joins the diff engine to Prisma.
 *
 * Two callers:
...
/**
 * Pull the shop's CartFixture+FixtureBaseline rows and
 * FunctionOutput rows, run the diff engine, persist the results
...
export async function runDriftAndPersist(shopDomain: string): Promise<DiffRunOutcome> {
  ...
  const baselines = fixtureRows
    ...
    .map((f) => ({
      ...
      // Segment context. Empty customerTagSignature → no B2B tag;
