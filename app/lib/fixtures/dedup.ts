  ...
  return {
    ...
    lastObservedAt:
      existing.lastObservedAt > next.lastObservedAt
        ...
        : next.lastObservedAt,
    // Baseline tracks the freshest order so Slice 6 diffs against the most
    // recent ground truth.
    baseline: newer.baseline,
    // Cart total: keep the freshest. Slice 6 may want a histogram instead but
    // single-value is enough for Slice 3.
