 ...
 * rendered on demand from the persisted snapshot — see Slice 4 README for
    ...
    <View style={styles.section}>
      ...
      {snapshot.topRisks.length > 0 ? (
        <View style={{ marginTop: 8 }}>
          <Text style={[styles.paragraph, { fontFamily: "Helvetica-Bold" }]}>Top risks</Text>
...
function Footer({ snapshot }: { snapshot: AuditSnapshot }) {
  return (
    <View style={styles.footer} fixed>
      <Text>
        Script Sentinel · {snapshot.shopDomain} · Audit generated {fmtDate(snapshot.generatedAt)}
      </Text>
      <Text
